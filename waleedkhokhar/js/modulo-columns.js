...
(function (window) {
  ...
  function moduloColumnsLayoutModeDefinition(LayoutMode) {
    ...
    ModuloColumns.prototype._resetLayout = function () {
      ...
      var cols = (this.cols =
        Math.floor((containerWidth + gutter) / this.columnWidth) || 1);
    ...
    ModuloColumns.prototype._getItemLayoutPosition = function (item) {
      ...
      var itemCols = Math.min(
        this.cols,
        Math.ceil(itemWidth / this.columnWidth)
      );
      ...
      for (var offset = 0; offset < itemCols; offset++) {
        maxHeight = Math.max(
          maxHeight,
          this.columnHeights[this.currentColumn + offset]
        );
      }
  ...
  if ("function" === typeof define && define.amd) {
    ...
    // Load synchronously
    moduloColumnsLayoutModeDefinition(window.Isotope.LayoutMode);
  }
})(window);
